...
class MEM:
    ...
    def MEM_WR(self,address,data):
        ...
        if address < 0xF000:
            ...
            print(format(data, '04x'),end="",flush=True)
            exit(0)
...
class nlp16a:
    ...
    def execute_inst(self):
        # 1st word load
        self.reg[0x0C] = self.reg[0x0D] #ADDR <- IP
        inst_head_address = self.reg[0x0D]

        if inst_head_address > self.program_area:
            ...
            print("    op address:","{:04X}".format(inst_head_address),file=sys.stderr)
